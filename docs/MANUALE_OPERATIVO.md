...
**Versione:** 0.16.0 (Configurazione Ibrida 3-Livelli)
...
## 👁️ 5. Visione e Interazione Multimodale (v0.15.2)
...
## 🎨 7. Generazione Immagini (v0.15.2)
...
4. Clicca **+ Add Override** per aggiungere una nuova regola. Il tag deve corrispondere esattamente al nome del plugin (es. `WEBCAM`, `IMAGE_GEN`).
5. Scrivi l'istruzione in linguaggio naturale — l'IA la riceverà testualmente in ogni prompt.
6. Clicca **Save Overrides** per salvare su disco.

### Come modificare lo YAML direttamente (Drive Editor)
1. Nella scheda Routing, clicca **📝 Edit in Drive**.
2. Si aprirà lo Zentra Code Editor (Monaco) con il file `routing_overrides.yaml` già caricato.
3. Salva con `Ctrl+S`.

### Esempi Pratici
```yaml
overrides:
  # Forza sempre l'uso della camera del telefono dell'utente
  WEBCAM: "Quando l'utente dice 'telefono', 'smartphone' o 'browser', usa SEMPRE target='client'."

  # Limita la generazione immagini a richieste esplicite
  IMAGE_GEN: "Usa generate_image SOLO se l'utente chiede esplicitamente di 'creare', 'disegnare' o 'generare' un'immagine."

  # Preferisci fonti tecniche nelle ricerche web
  WEB: "Dai priorità a documentazione tecnica, Wikipedia e fonti ufficiali. Evita aggregatori di notizie."
```

---
*Fine del rapporto documentale v0.16.0.*
