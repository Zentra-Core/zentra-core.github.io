...
# Standardize paths to the TRUE project root (C:\Zentra-Core)
def _find_root():
...
REGISTRY_PATH = os.path.join(_SELF_DIR, "..", "..", "core", "registry.json")
...
def generate_self_awareness(personality_name):
    try:
        ...
        core_dir = os.path.join(PROJECT_ROOT, "zentra", "core")
...
def generate_response(user_text, external_config=None, tag=None, images=None, agent_context=None, save_history=True):
    ...
    rp_active = False
    try:
        from zentra.plugins.roleplay.main import get_roleplay_prompt
