...
def check_folders():
    import zentra
    Z_DIR = os.path.dirname(zentra.__file__)
    # core, memory, plugins, app are inside zentra. personality, logs, ui are in root.
    folders = [
        os.path.join(Z_DIR, "plugins"), "personality", "logs", 
        os.path.join(Z_DIR, "memory"), os.path.join(Z_DIR, "core"), 
        "ui", os.path.join(Z_DIR, "app")
    ]
